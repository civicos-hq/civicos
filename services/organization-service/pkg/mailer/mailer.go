...
//
// Duplicated from identity-service rather than shared: each Go service is
// its own module, and the repo already duplicates pkg/config, pkg/database
// and pkg/response the same way. If a third service needs mail, that is the
// point to extract a shared module.
