  ...
  if (query.isError || !query.data) {
    return (
      <section className="space-y-4">
        <Link to="/projects" className="text-sm font-semibold text-civic-700 hover:underline">
          {t('projectDetail.back')}
        </Link>
        <p className="text-sm text-red-600">{t('projectDetail.loadError')}</p>
      </section>
    );
  }
  ...
  return (
    <section className="space-y-6">
      ...
      <article className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <dl className="grid grid-cols-1 gap-4 text-sm text-slate-600 md:grid-cols-2">
          <div>
      ...
      <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {!updatesQuery.isLoading && updates.length === 0 && (
          <p className="text-sm text-slate-600">{t('projectDetail.progressEmpty')}</p>
        )}
