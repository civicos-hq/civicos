...
function useFeed(communityId?: string) {
  return useQuery({
    queryKey: ['discover-feed', communityId ?? 'anon'],
    queryFn: async () => {
      const res = await api.get<ApiResponse<{ items: FeedItem[] }>>('/api/v1/discover/feed', {
...
export function DiscoverPage() {
  ...
  const feedQuery = useFeed(communityId);
  const items = feedQuery.data ?? [];

  // Group items by tier preserving the rank order.
          ...
          return (
            <section key={tier} className="space-y-3">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {tierItems.map((item) => (
                  <FeedCard
                    key={`${item.kind}-${item.issue?.id ?? item.petition?.id}`}
                    item={item}
                  />
                ))}
              </div>
